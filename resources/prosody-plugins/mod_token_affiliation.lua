...
local user_ic_roles = module:depends("user_ic_roles");
...
module:hook("muc-occupant-joined", function (event)
    ...
    if context_user then
        ...
        if context_user["ic_roles"] then
            for _, v in pairs(context_user["ic_roles"]) do
                local partner = nil;
                ...
                if v["role"] then
                    module:log("info", "Setting role %s for %s", v["role"], occupant.nick);
                    user_ic_roles.set_role(room.jid, occupant.nick, v["role"], partner)
                end
            end
        end
    end
